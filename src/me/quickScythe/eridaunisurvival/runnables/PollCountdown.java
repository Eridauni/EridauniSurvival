...
public class PollCountdown implements Runnable {
	...
	public void run() {
		if(i > 1){
			Bukkit.broadcastMessage(CoreUtils.colorize("&e&lPoll&f >&7 The current poll is ending in &f" + i + "&7 seconds..."));
			Bukkit.getScheduler().runTaskLaterAsynchronously(plugin, new PollCountdown(plugin, i-1), 20);
		}
		if(i == 1){
			Bukkit.broadcastMessage(CoreUtils.colorize("&e&lPoll&f >&7 The current poll is ending in &f1&7 second..."));
			Bukkit.getScheduler().runTaskLaterAsynchronously(plugin, new PollCountdown(plugin, i-1), 20);
		}
		if(i == 0){
			Bukkit.broadcastMessage(CoreUtils.colorize("&e&lPoll&f >&7 The current poll has ended."));
